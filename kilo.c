...
void updateWindowSizes(){
    ...
    //to-do: also update other windows when they become usable
}
...
void editorRefreshScreen(){
    ...
    abAppend(&ab, "\x1b[H", 3);  //moves cursor position
    ...
    moveCursor(&ab, E.rx - E.colOff + 1, E.cy - E.rowOff + 1);
...
void editorDrawStatusBar(struct abuf * ab){
    //manually move to end of screen
    moveCursor(ab, 0, E.fullScreenRows - 1);
...
void drawBorder(struct abuf * ab){
    moveCursor(ab, E.screenCols + 1, 0);
    for (int y = 0; y < E.screenRows; y++) {
        abAppend(ab, "|\x1b[D\x1b[B", 7);
    }
    moveCursor(ab, 0, E.screenRows + 1);
